...
def query_text(query):
    ...
    # или задумался после ввода первого числа
        ...
        text = 'Ты хуй - просто хуй\n Введи ещё число'
        xui = types.InlineQueryResultArticle(id='1', title="По жизни", # Описание отображается в подсказке,
            # message_text - то, что будет отправлено в виде сообщения
            description="Результат: {}".format(text), input_message_content=types.InputTextMessageContent(
                message_text="{}".format(text)), # Указываем ссылку на превью и его размеры
            thumb_url=plus_icon, thumb_width=48, thumb_height=48)
        bot.answer_inline_query(query.id, [xui], cache_time=0)
        #return
    ...
    try:
        ...
        bot.answer_inline_query(query.id, [r_sum, r_sub, r_div, r_mul], cache_time=21)
